...
const Home = () => {
        ...
        productService.getall().then(
            ...
            OderFireBaseService.getAll().then(
                ...
                let data = []
                    for (let i = 0; i <= 16; i++) {
                        if (i != 0) {
                            data = newarry.filter(item => item.soban == i).sort((a, b) => a.trangthai - b.trangthai)
    ...
    const deleteByIndex = (e, statusoder) => {
        if (statusoder == 1) {
            ...
            setlistOdering(listOdering.filter(item => item.product.productID != tam.product.productID))
        }
        else {
        
            oderService.delete(listcash[e].oderId).then(res => {
             
                let listcashnew=listcash.filter(item => item.oderId != listcash[e].oderId)
                setlistcash(listcashnew)
                setSumcash(listcashnew.reduce((a, v) => a = a + (v.soluong * findproduct(v.productId).price), 0))
    ...
    const SelectTable = (e) => {
        ...
        oderService.getbyban(e).then(res => {
            ...
            if (res.data.length > 0) {
                ...
                setSumcash(res.data.reduce((a, v) => a = a + (v.soluong * findproduct(v.productId).price), 0))
            }
            else {
                setContent([]
                )
        ...
        settable(e)
        setshow(true)
        let listnote = localStorage.getItem('note') ? JSON.parse(localStorage.getItem('note')) : []
    ...
    const OnclickOder = () => {
        ...
        listOdering.map((item, index) => {
            let order = {  gio: time, ngay: date, productId: item.product.productID ,soban: ms.ban, soluong: item.soluong, trangthai: ms.trangthai}
            if (listoffirebane.findIndex(e => e.productId == item.product.productID) == -1) {
        ...
        ms.mes.map((item, i) => {
            ...
            let order = { soban: ms.ban, trangthai: ms.trangthai, soluong: item.soluong, productId: item.product.productID, ngay: date, gio: time }
            ...
            if (listbyban.length == 0) {
                ...
                if (vitri == -1) {
                    ...
                    if (listbyban[vitri].trangthai == 3) {
                        listbyban[vitri].soluong = Number(item.soluong) + Number(listbyban[vitri].soluong)
                        let orders = { soban: ms.ban, trangthai: ms.trangthai, soluong: listbyban[vitri].soluong, productId: item.product.productID, ngay: date, gio: time }
        ...
        OderFireBaseService.getAll().then(
            res => {
                ...
                let listOlder = []
                listOlder = Object.values(res.data).filter(e => e.id == table)[0] ? Object.values(res.data).filter(e => e.id = ms.ban)[0].data : []
    ...
    const OnclickCash = () => {
        ...
        setlistshow(lisset)
        cashService.update({ date: date, id: listshow[ms.ban].mes[0].oderId, data: listshow[ms.ban].mes }).then(
            res => {
                toast.success("ok")
                cashService.getbyday(date).then(
                    ress => {
                        let newarry=[]
    ...
    const changeProduct = (e) => {
        ...
        let index = listOderings.findIndex(item => item.product.productID == e[0].productID)
    ...
    const changesoluong = (e, index, trangthai) => {
        ...
        oderService.edit(newtable.oderId, newtable).then(
            res => {
              
            }
        )
        if (trangthai == 3) {
            setSumcash(contentnew.reduce((a, v) => a = a + (v.soluong * findproduct(v.productId).price), 0))
                    ...
                    <div className="container" data-aos="fade-up">
                                        ...
                                        <>
                                            {
                                                index != 0 && (
                                                    item.trangthai ? <>
                                                        {
                                                            item.trangthai == 3 ?
                                                                <div onClick={(e) => SelectTable(index)} className="col-lg-3 col-md-6 col-sm-6 border ban align-middle ups" >Bàn {index} {getNoteByNumberTable(index) ? '(' + getNoteByNumberTable(index) + ')' : ''}
                                                                </div> :
                                                                <div onClick={(e) => SelectTable(index)} className="col-lg-3 col-md-6 col-sm-6 border ban align-middle " >Bàn {index} {getNoteByNumberTable(index) ? '(' + getNoteByNumberTable(index) + ')' : ''}
                                                                </div>
                                                        }
                                                    </>
                                                        :
                                                        <div onClick={(e) => SelectTable(index)} className="col-lg-3 col-md-6 col-sm-6 border ban align-middle " >Bàn {index}
                                                        </div>
                                                )
                        ...
                        >
                            ...
                            <Modal.Body >
                                ...
                                {
                                    listOdering && listOdering.length > 0 && listOdering.map((item, index) => {
                                        return (
                                            <>
                                                <div className='row'>
                                                    ...
                                                    <div className='col col-lg-2 border' style={{ textAlign: "center" }}>
                                                        <input type="number" className="form-control  border-radius" onChange={(e) => changesoluong(e, index, item.trangthai)} value={item.soluong} />
...

                                {
                                    ...
                                    listcash && listcash.length > 0 && listcash.map((item, index) => {
                                        return (
                                            <>
